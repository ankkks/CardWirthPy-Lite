...
廃止オプション
・一般「デバッグ中はレベル上昇を抑止する」…デバッグモードでは常にレベルアップしない
・描画「白いカーソルを使用する」…白カーソルで固定
・シナリオ「シナリオ選択ダイアログへシナリオをドロップした時は～」…そのままシナリオを開く
・シナリオ「シナリオのインストールに成功したら～」…元ファイルは削除しない
・詳細「拠点ごとにスキンを記憶しない」…常に拠点のスキンを記憶
・詳細「スター付きカードの売却破棄を禁止しない」…常に禁止
・詳細「召喚獣カードの拡大率を大きくする」…CWの拡大率で固定
