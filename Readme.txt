...
17.05.07 BACKSLASHとBACKSPACE対応のダイアログを増やした
17.05.06 設定ダイアログとスキンダイアログのレイアウトを調整
17.05.05 解説ダイアログの大きさをCWに準拠
17.05.03 設定ダイアログの文言をCWの表記に統一(メッセージウィンドウ→メッセージボードなど)
17.05.02 自動登録のキー割り当てをCtrl+Aに変更
17.05.01 設定ダイアログの簡易モードを廃止
17.04.30 宿選択・宿帳でCtrl+N(新規作成)、宿帳でCtrl+R(自動登録)に対応
17.04.29 Rebootに合わせてキャラ情報の「状態」に表示される状態異常名はスキン文言を読む
...
・シナリオ選択ダイアログ関係の変更
 ...
 2.一覧表示にしている場合はツリー側のレベル表示([1～3]など)を非表示にするようにした(狭くてタイトルが表示しきれない為)

ショートカットキーの強化
#################################################################################################################
　拠点選択　　　新規宿(CTRL+N)/削除(DELETE)/閉じる(BACK)
　キャラ選択　　新規登録(CTRL+N)/自動登録(CTRL+A)/削除(DELETE)/閉じる(BACK)
　カード操作　　ゴミ箱に送る(DELETE)
　シナリオ選択　インストール(CTRL+I)/新規フォルダ(CTRL+N)/移動(CTRL+M)/名前の変更(CTRL+R)/削除(DELETE)/戻る(BACK)
　パーティ記録　削除(DELETE)/閉じる(BACK)
#################################################################################################################
