...
12/17 まで　Wsn.2　高解像度、KC分岐「手札」を除外(全て/アイテムでの手札検索はOK)して取り込み

●動作保証環境
Windows 7/10 64bit

●インストール/アンインストール
CardWirthPyLite.exeを起動して下さい。CWと同じくレジストリを使用しないのでフォルダごと削除でOK。
シナリオを沢山入れて重いという場合は、設定＞シナリオ＞シナリオタイプ＞データベース構築を推奨。

PyRebootとの互換
[宿/スキン]
1.0~最近のテスト版環境でシェアする分にはほぼ問題ないはず。WSN2確定後は保証できません。
なにか問題が起こった場合は一度1.50に変換すれば大丈夫だと思います。(Py専用の諸々は初期化されますが)
[シナリオ]
Py1.1ベースなのでCW1.20～1.50仕様/WSN1に対応します。具体的には1.50付属の旧WBかXEditor4.1で作ればOKです。
...
・Rebootでメニューカードと効果系カードのフォントサイズが異なっているのを一致させるようにした(バグ？)
・Rebootでメニューバーのエリア移動/戦闘中断が表示されていないのは恐らくバグだが、再setが効かないので固定にした
